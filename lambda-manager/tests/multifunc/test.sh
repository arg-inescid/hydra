...
curl -s -X POST $LAMBDA_MANAGER_HOST:$LAMBDA_MANAGER_PORT/upload_function?username=user\&function_name=jsupow\&function_language=javascript\&function_entry_point=main\&function_memory=$FUNCTION_MEMORY\&function_runtime=openwhisk\&function_isolation=true\&invocation_collocation=false -H 'Content-Type: application/octet-stream' --data-binary "$ARGO_HOME/benchmarks/src/javascript/cr-uploader/init.json"
...
curl -s -X POST $LAMBDA_MANAGER_HOST:$LAMBDA_MANAGER_PORT/upload_function?username=user\&function_name=pycoow\&function_language=python\&function_entry_point=main\&function_memory=$FUNCTION_MEMORY\&function_runtime=openwhisk\&function_isolation=true\&invocation_collocation=false -H 'Content-Type: application/octet-stream' --data-binary "$ARGO_HOME/benchmarks/src/python/cr-compression/init.json"

...
curl -s -X POST $LAMBDA_MANAGER_HOST:$LAMBDA_MANAGER_PORT/upload_function?username=user\&function_name=jvhwow\&function_language=java\&function_entry_point=Hello\&function_memory=$FUNCTION_MEMORY\&function_runtime=openwhisk\&function_isolation=true\&invocation_collocation=false -H 'Content-Type: text/plain' --data "$ARGO_HOME/benchmarks/src/java/cr-hello-world/init.json"

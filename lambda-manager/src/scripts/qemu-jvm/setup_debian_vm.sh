...
# Note: we need to reload this var because we would overwrite it in previous source
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
...
cat $DIR/id_rsa.pub | sudo tee stretch/root/.ssh/authorized_keys

# Build a disk image.
dd if=/dev/zero of=stretch.img bs=1M seek=2048 count=1

...
echo "\$JAVA_HOME/bin/java -Djava.library.path=\$JAVA_HOME/lib -agentlib:native-image-agent=config-output-dir=config,caller-filter-file=caller-filter-config.json -jar $FUNCTION_NAME.jar ${@:8}" > $LAMBDA_HOME/shared/run.sh

# TODO - I am currently hardcoding the --console. We might want to be able to remove avoid it.
$LAMBDA_HOME/debian-vm_unikernel.sh --memory $LAMBDA_MEMORY --gateway $LAMBDA_GATEWAY --ip $LAMBDA_IP --mask $LAMBDA_MASK --kernel $KERNEL_PATH --img $LAMBDA_HOME/stretch.img --shared $LAMBDA_HOME/shared --tap $LAMBDA_TAP --console

...
{   
    ...
    void
execute_function(char *string1, char* string2)
{
    char* home = getenv("ARGO_HOME");
    if (home!= NULL) {
	// Concatenate the strings
	char tmp_dir[strlen(home) + strlen("/graalvisor/build/libs/") + strlen(string1) + 1]; // +1 for null terminator
	strcpy(tmp_dir, home);
	strcat(tmp_dir, "/graalvisor/build/libs/");
	strcat(tmp_dir, string1);
	void *handle;
	void (*hello)();
	// Open the shared object file
	handle = dlopen(tmp_dir, RTLD_LAZY);
	if (!handle) {
	    fprintf(stderr, "%s\n", dlerror());
	    exit(EXIT_FAILURE);
	}
...
process_setup(const char *fifo_path) {
	...
	if (token != NULL) {
	    ...
	    perror("No string2 found");
	    exit(EXIT_FAILURE);
	}
    } else {
	perror("No string1 found");
	exit(EXIT_FAILURE);
    }
    execute_function(string1, string2);
    free(string1);
    free(string2);
}
